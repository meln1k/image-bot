...
enum Command {
    #[command(description = "Image ranking bot, type /start to launch it")]
    Help,
    ...
    if let Some(text) = msg.text() {
        match BotCommands::parse(text, me.username()) {
            ...
            Ok(Command::Start) => {
                ...
                send_new_picture(bot, msg.chat.id).await?;
                // let keyboard = make_keyboard();
                // bot.send_photo(msg.chat.id, InputFile::memory(cat_pic))
                //     .caption("Hello, I'm a bot to show you a cat picture. Please, choose a reaction:")
                //     .reply_markup(keyboard)
                //     .await?;
            }
...
async fn callback_handler(bot: Bot, q: CallbackQuery) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    if let Some(version) = q.data {
        let text = format!("feedback: {version}");
        ...
        if let Some(Message { id, chat, .. }) = q.message {
            bot.edit_message_caption(chat.id, id).caption(text).await?;
            send_new_picture(bot, chat.id).await?;
